...
for i=1:num
    ...
    for j = 1:20
        ...
        U(:,1) = x0;
        for pp=1:length(t)-1
            U(:,pp+1) = U(:,pp) + dt*F(t(pp),U(:,pp)) + sqrt(dt)*[sqrt(2*0.5);0.02*sqrt(alpha(U(1,pp))*(1-U(2,pp))+beta(U(1,pp))*U(2,pp))].*randn(2,1);
        end
...
writematrix(SOL,'solutions_stoch.txt');


%% make the movie

%load in data
SOL = load('solutions_stoch.txt');
...
%make the object
v = VideoWriter('isochrons_stoch.mp4','MPEG-4');
open(v)
...
for k=1:50:length(SOL)
    ...
    xlabel('v')
    ylabel('n')
    title('Noisy Morris Lecar System')
